...
with open('FirstDate_ToBeTrained.txt','w') as f_out: # output facts.txt
    with open('C:/Users/lindu/Desktop/Winter 2017 Classes/CS249/Project/Code/data-train-dca/Train_features.txt') as f_in:
        for line in tqdm(f_in):
            part = line.strip().split('\t')
            user1 = part[0]
            user2 = part[1]
            min_diff = (facts[user1])[0] - (facts[user2])[0]
            min_diff_abs = abs(min_diff)
            duration = timedelta(microseconds=min_diff_abs)
            t = str(user1) + ',' + str(user2) + ',' + str(duration)
            f_out.write(t+'\n')
